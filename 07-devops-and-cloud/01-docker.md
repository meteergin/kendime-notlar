## Konu 37: Containerization & Orchestration (Docker & Kubernetes)

Modern yazılım geliştirme süreçlerinde, uygulamaların tutarlı bir şekilde paketlenmesi (Docker) ve yönetilmesi (Kubernetes) standart hale gelmiştir. Senior bir geliştirici olarak, sadece "image build" etmeyi değil, container optimizasyonunu, Kubernetes mimarisini ve deployment stratejilerini bilmelisiniz.
...
#### Neden Docker?
*   **Consistency:** "Benim makinemde çalışıyordu" sorununu çözer. (Dev = Test = Prod).
*   **Isolation:** Her uygulama kendi bağımlılıklarıyla (Java versiyonu, kütüphaneler) izole çalışır.
*   **Portability:** Her yerde (AWS, Azure, Local) aynı şekilde çalışır.

#### Docker Mimarisi
*   **Docker Daemon:** Arka planda çalışan, container'ları yöneten servis.
*   **Docker Image:** Uygulamanın çalışması için gereken her şeyi (kod, runtime, kütüphaneler) içeren salt okunur (read-only) şablon.
*   **Docker Container:** Image'ın çalışan (runnable) halidir.
*   **Registry:** Image'ların saklandığı depo (Docker Hub, AWS ECR).

#### Best Practices & Optimizasyon
1.  **Multistage Builds:** Build aşaması (Maven, JDK) ile Run aşamasını (JRE) ayırın. Image boyutunu %90 küçültür.
    ```dockerfile
    # Stage 1: Build
    ...
    ```
2.  **Layer Caching:** Değişmeyen katmanları (bağımlılıklar) yukarı, sık değişenleri (kod) aşağı koyun.
3.  **Distroless Images:** İçinde shell bile olmayan, sadece uygulama için gereken minimum dosyaları içeren image'lar (Güvenlik için).

---

### 2. Kubernetes (K8s - Orchestration)

Docker tek bir makinede container yönetir. Kubernetes ise binlerce sunucuda (cluster) binlerce container'ı yönetir.

#### Temel Bileşenler (K8s Objects)
*   **Pod:** K8s'in en küçük birimi. İçinde bir veya daha fazla container çalışır. (Genelde 1 Pod = 1 Container).
*   **Deployment:** Pod'ların replikasyonunu, update stratejisini (Rolling Update) ve sağlığını yönetir. "Bana 3 tane User Service Pod'u çalıştır" dersiniz, o sağlar.
*   **Service:** Pod'lara sabit bir IP ve DNS adı verir. Load balancing yapar.
    *   *ClusterIP:* Sadece cluster içi erişim.
    *   *NodePort:* Dış dünyadan erişim (port açarak).
    *   *LoadBalancer:* Cloud provider'ın load balancer'ını kullanır.
*   **Ingress:** Dış dünyadan gelen HTTP/HTTPS trafiğini servislere yönlendirir (Reverse Proxy).
*   **ConfigMap & Secret:** Konfigürasyon ve hassas verileri (şifreler) pod'dan ayırır.
*   **Volume (PV/PVC):** Veri kalıcılığı (Persistence) sağlar. Pod ölse bile veri kaybolmaz.

#### K8s Mimarisi
*   **Control Plane (Master Node):** Beyin.
    *   *API Server:* Tüm isteklerin geldiği kapı.
    *   *Etcd:* Cluster durumunu tutan key-value store (Single Source of Truth).
    *   *Scheduler:* Pod'u hangi node'a koyacağına karar verir.
    *   *Controller Manager:* İstenen durum (Desired State) ile mevcut durumu eşitler.
*   **Worker Node:** Kas gücü.
    *   *Kubelet:* Master'dan emir alır, container'ı çalıştırır.
    *   *Kube-proxy:* Network kurallarını yönetir.

#### Kritik Mülakat Soruları

**Soru 1: Pod Lifecycle nedir? `Pending`, `Running`, `CrashLoopBackOff` ne anlama gelir?**
*   **Pending:** Scheduler henüz uygun node bulamadı (Kaynak yetersiz olabilir).
*   **Running:** Container çalışıyor.
*   **CrashLoopBackOff:** Container sürekli çöküp yeniden başlıyor (Uygulama hatası, konfigürasyon hatası).

**Soru 2: Sidecar Pattern nedir?**
*   **Cevap:** Ana container'ın yanına yardımcı bir container koymaktır.
*   *Örnek:* Log toplayıcı (Fluentd), Service Mesh proxy (Istio Envoy). Ana uygulama logu dosyaya yazar, sidecar o dosyayı okuyip ElasticSearch'e gönderir.

**Soru 3: Headless Service nedir?**
*   **Cevap:** Load balancing yapmayan, doğrudan Pod IP'lerini dönen servistir. (`ClusterIP: None`).
*   *Kullanım:* StatefulSet'lerde (Database cluster) master/slave ayrımı için pod'lara doğrudan ulaşmak gerekir.
